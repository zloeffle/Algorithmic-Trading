...
class Trader:           
    ...
    def create_features(self,tickers,start_date,end_date):
        ...
        for t in tickers:
            data = yf.download(t,end=end_date,period='1y')
            ...
            df.loc[t,'Profit'] = int(end_price > start_price)
    ...
    def mlp(self,data):
        ...
        train = self.create_features(data,start,end)
        test = self.create_features(data,start1,end1)

        cols = train.columns
        inputs = train[cols[:-1]].to_numpy()
        targets = train['Profit']
        model = MLP(inputs,targets)
        model.train()

        inputs = test[cols[:-1]].to_numpy()
        out = [model.predict(p) for p in inputs]
        test['Pred'] = out
        print(test)
